class CreateDbController < ApplicationController

NOMINAL = "名目"
REAL = "実質"

def test
    get_data_gdp("0003109766",REAL,CategoryList.all,REALSnaQuarterly.all) #国民経済計算（四半期＿実質
  ...
  def get_data_gdp(stats_data_id,period_time,db_catlist,db_sna)
    ...
    tb_title = "test" #data_all[:GET_STATS_DATA][:STATISTICAL_DATA][:TABLE_INF][:TITLE]
    ...
    if db_catlist.find_by(category_code: cat_code).category_update_date != db_sna[db_sna.count-1].update_date
